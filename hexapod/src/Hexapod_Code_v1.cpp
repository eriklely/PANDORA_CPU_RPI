...
volatile bool g_fFaceDetectionEnabled = false;  // face detection off on start; toggled by Recognition Mode (button bit 14)
...
void setup() {
    ...
    std::thread camera_thread(hexapod_rpicam_main, 0, nullptr);
    camera_thread.detach();  // runs forever in background    
}
...
void Gait(byte GaitCurrentLegNr) {
    ...
    if ((TravelRequest && (g_InControlState.gaitCur.NrLiftedPos & 1) && LegStep == 0) ||
        ...
        GaitPosX[GaitCurrentLegNr] -= (g_InControlState.TravelLength.x / (short)g_InControlState.gaitCur.TLDivFactor);
        GaitPosY[GaitCurrentLegNr] = 0;
        GaitPosZ[GaitCurrentLegNr] -= (g_InControlState.TravelLength.z / (short)g_InControlState.gaitCur.TLDivFactor);
        GaitRotY[GaitCurrentLegNr] -= (g_InControlState.TravelLength.y / (short)g_InControlState.gaitCur.TLDivFactor);
    }
}
