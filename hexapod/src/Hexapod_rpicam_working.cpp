...
static void requestComplete(Request *request) {
    ...
    for (const auto &pair : buffers) {
        ...
        // - Balanced (current): scaleFactor=1.2, minNeighbors=5, minSize=cv::Size(30,30)
        face_cascade.detectMultiScale(gray, frontal_faces, 1.1, 3, 0, cv::Size(30, 30));

        // Detect profiles/sides (tuned stricter to reduce noise)
        profile_cascade.detectMultiScale(gray, profile_faces, 1.1, 3, 0, cv::Size(30, 30));
