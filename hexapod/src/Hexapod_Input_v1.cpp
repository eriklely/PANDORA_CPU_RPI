...
unsigned long g_ulLastMsgTime = (unsigned long)-1; // Tijd van het laatste ontvangen bericht
...
void sendUdpOnly(const char* format, ...) {
    ...
    vsnprintf(response, sizeof(response), format, args); // Formatteer het bericht
    va_end(args);

    // Controleer of het bericht een fout bevat (zoals "error", "failed", etc.)
    const char* error_keywords[] = {"error", "failed", "invalid", "cannot"};
    bool is_error = false;
    for (const char* keyword : error_keywords) {
        if (strstr(response, keyword) != nullptr) {
            is_error = true;
            break;
        }
    }

    // Controleer of het bericht "command" of "Command" bevat
    const char* command_keywords[] = {"Command", "command"};
    bool contains_command = false;
    for (const char* keyword : command_keywords) {
        if (strstr(response, keyword) != nullptr) {
    ...
    // Verzend via UDP als debuggen is ingeschakeld of als het bericht een commando bevat
    if (clientAddr.sin_port != 0 && sendUdpSocket >= 0) {
        if (g_fDebugEnabled || contains_command) {
            struct sockaddr_in reply_addr = clientAddr;
            reply_addr.sin_port = htons(SEND_UDP_PORT); // Bijv. poort 8889
...
//=================================================================================================
            ...
            if (calculated_checksum == packet.checksum) {
                ...
                switch (packet.special_command) {
                    ...
                    case 0: { // Normaal commando
                        ...
                        // Beperk aswaarden tot geldige bereik
                        sustained_axes_lx = -std::min(std::max(packet.sustained_axes_lx, static_cast<int16_t>(-32768)), static_cast<int16_t>(32767));
                        sustained_axes_ly = -std::min(std::max(packet.sustained_axes_ly, static_cast<int16_t>(-32768)), static_cast<int16_t>(32767));
                        sustained_axes_rx = std::min(std::max(packet.sustained_axes_rx, static_cast<int16_t>(-32768)), static_cast<int16_t>(32767));
                        sustained_axes_ry = std::min(std::max(packet.sustained_axes_ry, static_cast<int16_t>(-32768)), static_cast<int16_t>(32767));
                        sustained_axes_sx = -std::min(std::max(packet.sustained_axes_sx, static_cast<int16_t>(-32768)), static_cast<int16_t>(32767));
                        sustained_axes_sy = -std::min(std::max(packet.sustained_axes_sy, static_cast<int16_t>(-32768)), static_cast<int16_t>(32767));
    ...
    if (g_ulLastMsgTime != (unsigned long)-1 && (currentTime - g_ulLastMsgTime > TIMEOUT_MS)) { //veranderd ================================================================================================================
        sustained_axes_lx = sustained_axes_ly = sustained_axes_rx = 
        sustained_axes_ry = sustained_axes_sx = sustained_axes_sy = 0;
...
if (ControlMode == CAMERAMODE) {
    ...
    const int    MAX_NO_FACE_FRAMES = 24;   // ~2 seconds at ~12 FPS

    double x_offset = 0.0;   // normalized: -1 (face left) to +1 (face right)
    double y_offset = 0.0;   // normalized: -1 (face above) to +1 (face below)
    ...
    if (faceDetected && center_x > 0 && center_y > 0) {
        x_offset = (current_faces.centroid_x - center_x) / center_x;
        y_offset = (current_faces.centroid_y - center_y) / center_y;
        no_face_count = 0;
    }

    // PI gains for normalized offset [-1, 1]: same response regardless of VIDEO_STREAM_WIDTH/HEIGHT
    const double Kp_turn = 26.0;
    const double Kp_tilt = 10.0;
    const double Ki_turn = 17.0;
    const double Ki_tilt = 8.0;
    const double MAX_INTEGRAL_NORM = 5.0;

    if (faceDetected || no_face_count <= MAX_NO_FACE_FRAMES) {
        if (faceDetected) {
            integral_turn += x_offset;
            integral_tilt += y_offset;
            ...
            control_turn = Kp_turn * x_offset + Ki_turn * integral_turn;
            control_tilt = Kp_tilt * y_offset + Ki_tilt * integral_tilt;
        } else {
