...
std::atomic<bool> g_fServosFree{true};    // thread-safe: read/written from main and input threads
...
void ServoDriver::Init(void) {
    ...
    for (int i = 0; i < NUMSERVOS; i++) {
        ...
        if (w != 0xffff) {
            // Servo found — configure compliance slope
...
word ServoDriver::GetBatteryVoltage(void) {
    ...
    if (uldt > VOLTAGE_MAX_TIME_BETWEEN_CALLS || 
        ...
        if (g_id_controller == 200)
            ...
            if (g_bLegVoltage >= CNT_LEGS) g_bLegVoltage = 0;  // bounds check before index
...
void ServoDriver::OutputServoInfoForTurret(short sTiltAngle1, short sPanAngle1) {
    ...
    wTiltSDV = (((long)sTiltAngle1 * TURRET_TILT_SCALE) * cPwmMult) / cPwmDiv + cPFConst;
    wPanSDV  = (((long)sPanAngle1  * TURRET_PAN_SCALE)  * cPwmMult) / cPwmDiv + cPFConst;
