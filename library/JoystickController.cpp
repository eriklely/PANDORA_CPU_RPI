...
#else
#define PRINT(...) do {} while (0)  // No-op als USEPRINTF niet defined is
#endif
...
void *LinuxJoy::JoystickThreadProc(void *pv)
{
    ...
    while(!pljoy->cancel_thread_)
    {
        ...
        {    
            ...
            { 
                ...
                {
                   PRINT("Error reading from Joystick %d\n", errno);
    ...
    PRINT("LinuxJoy - LinuxJoy thread exit\n");
    return 0;
}
...
bool LinuxJoy::begin(char *pszDevice)
{
    ...
    {
        PRINT("malloc of device name failed\n");
    ...
    if (err != 0)
    {
        PRINT("pthread_create failed with error: %d\n", err);
        free(input_device_name_); // Clean up allocated memory
        pthread_barrier_destroy(&thread_barrier_); // Clean up barrier
        return false;
    }

    // Sync startup (only reached if thread creation succeeds)
